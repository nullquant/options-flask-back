...
# black_scholes(option_type, fs, x, t, r, v)
# merton(option_type, fs, x, t, r, q, v)
# black_76(option_type, fs, x, t, r, v)
# garman_kohlhagen(option_type, fs, x, t, b, r, rf, v)
# asian_76(option_type, fs, x, t, t_a, r, v)
# kirks_76(option_type, f1, f2, x, t, r, v1, v2, corr)
# american(option_type, fs, x, t, r, q, v)
# american_76(option_type, fs, x, t, r, v)

# Implied Volatility Formulas 

# euro_implied_vol(option_type, fs, x, t, r, q, cp)
# euro_implied_vol_76(option_type, fs, x, t, r, cp)
# amer_implied_vol(option_type, fs, x, t, r, q, cp)
...
# b              Cost of Carry This is only found in internal implementations, 
#                   but is identical to the cost of carry (b) term commonly found in academic option pricing literature
# q              Continuous Dividend Used in Merton and American models; 
#                   Internally, this is converted into cost of carry, b, with formula b = r - q
# rf             Foreign Interest Rate Only used GK model; this functions similarly to q
# t_a            Asian Start Used for Asian options; This is the time that starts the averaging period 
#                   (t_a=0 means that averaging starts immediately). As t_a approaches t, the Asian value should become 
#                   very close to the Black76 Value
...
# The class for calculating Bachelier option prices and greeks

# Inputs: option_type = "p" or "c", fs = price of underlying, x = strike, t = time to expiration, r = risk free rate
#         b = cost of carry, v = implied volatility
# Outputs: value, delta, gamma, theta, vega, rho
def _bachelier(option_type, fs, x, t, r, b, v):
    logging.debug("Debugging Information: _bachelier()")
    # -----------
    # Test Inputs (throwing an exception on failure)
    _gbs_test_inputs(option_type, fs, x, t, 0, 0, v)
    ...
    if option_type == "c":
        # it's a call
        logging.debug("     Call Option")
        value = (fs - x) * norm.cdf(d) + v * t__sqrt * norm.pdf(d)
        delta = norm.cdf(d)
        gamma = norm.pdf(d) / (v * t__sqrt)
        theta = - norm.pdf(d) * v / (2 * t__sqrt)
        vega = t__sqrt * norm.pdf(d)
        rho = 0
    else:
        # it's a put
        logging.debug("     Put Option")
        value = (x - fs) * norm.cdf(-d) + v * t__sqrt * norm.pdf(d)
        delta = -norm.cdf(-d)
        gamma = norm.pdf(d) / (v * t__sqrt)
        theta = - norm.pdf(d) * v / (2 * t__sqrt)
        vega = t__sqrt * norm.pdf(d)
...
def _gbs_implied_vol(option_type, fs, x, t, r, b, cp, precision=.00001, max_steps=1000):
    return _newton_implied_vol(_gbs, option_type, x, fs, t, b, r, cp, precision, max_steps)

# ----------
# Find the Implied Volatility of an European (GBS) Option given a price
# using Newton-Raphson method for greater speed since Vega is available
def _bachelier_implied_vol(option_type, fs, x, t, r, b, cp, precision=.00001, max_steps=1000):
    return _newton_implied_vol(_bachelier, option_type, x, fs, t, b, r, cp, precision, max_steps)
...
def _american_implied_vol(option_type, fs, x, t, r, b, cp, precision=.00001, max_steps=1000):
...
def _newton_implied_vol(val_fn, option_type, x, fs, t, b, r, cp, precision=.00001, max_steps=1000):
...
def _bisection_implied_vol(val_fn, option_type, fs, x, t, r, b, cp, precision=.00001, max_steps=1000):
    logging.debug("-----")
    ...
    while (diff > precision) and (current_step < max_steps):
        ...
        logging.debug("     IVOL {0}. V[{1},{2},{3}. CP[{4},{5}]".format(current_step, v_low, v_mid, v_high, cp, cp_mid))
