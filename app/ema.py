...
# Exponential Moving Average by Jake Walden
...
# This function is equivalent to pandas' ewm(adjust=False).mean(), but much faster. ewm(adjust=True).mean() 
...
# Here is my fastest solution with no precision problems, nearly fully vectorized. It's gotten a little complicated 
# but the performance is great, especially for really huge inputs. Without using in-place calculations (which is 
# possible using the out parameter, saving memory allocation time): 3.62 seconds for 100M element input vector, 
# 3.2ms for a 100K element input vector, and 293µs for a 5000 element input vector on a pretty old PC 
# (results will vary with different alpha/row_size values).

def ema(data, window):
    return ewma_vectorized_safe(data, 2.0/(window + 1.0))
...
def keltner_channel(candles):
    ...
    EMA = ema.ema(close, 20)
    high = np.array([float(row[3]) for row in candles])
    low = np.array([float(row[2]) for row in candles])
